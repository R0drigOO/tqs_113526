...
public class Dip {
    ...
    public Dip(int[] arrayOfNumbers, int[] arrayOfStars) {
        ...
        if (!Arrays.stream(arrayOfNumbers).allMatch(nr -> nr >= 1 && nr <= NUMBERS_RANGE_MAX)) {
            throw new IllegalArgumentException("Numbers set contains values outside the expected range (1-" + NUMBERS_RANGE_MAX + ")");
        }
    
        if (!Arrays.stream(arrayOfStars).allMatch(nr -> nr >= 1 && nr <= STARS_RANGE_MAX)) {
            throw new IllegalArgumentException("Stars set contains values outside the expected range (1-" + STARS_RANGE_MAX + ")");
        }
    
        if (arrayOfNumbers.length != NUMBERS_REQUIRED || arrayOfStars.length != STARS_REQUIRED) {
